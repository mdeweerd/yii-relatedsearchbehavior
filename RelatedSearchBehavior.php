 ...
 *                        ),
 *                    /* Next line describes a field we do not search, but we define it here for convenience \*\/
...
class RelatedSearchBehavior extends CActiveRecordBehavior
{
     ...
     */
    public function getDataProvider($criteria=null,$options=[])
    {
        if ($criteria===null) {
     ...
     * @param array       $options  Options for the dataprovider.
     * @param bool        $useGET   Use data from $_GET variable (for sorting)
     *
     * @return KeenActiveDataProvider
     */
    public function relatedSearch($criteria,$options=[],$useGET=true)
    {
        ...
        foreach ($required_relations as $relation) {
            ...
            foreach (array_reverse(preg_split('/\./', $relation)) as $r) {
                if ($w!==null) {
                    $w=[$r=>['with'=>$w,'joinType'=>'LEFT JOIN', 'select'=>[]]];
                } else {
                    $w=[$r=>['select'=>[]]];
                }
            }
        ...
        foreach ($relations as $var=>$relationvar) {
            if (in_array(strtolower($var), ['owner','enabled'])) {
            ...
            // d. If a relation is used in the criteria condition, require it
            // and adjust the condition Pattern to search for, search if
            // relation is used as whole word only (don't want partials to be
            // replaced)
            $pattern = '/(?<![.`\'"])\s*\b('.preg_quote($var).')\b/';
            $totalReplaced = 0;
    ...
    public function __get($key)
    {
        ...
        throw new CException(
            Yii::t(
                ...
                ['{class}'=>get_class($this), '{property}'=>$key]
            )
        );
    }
     ...
     *
     * Similar to CDbCriteria::addBetweenCondition
     *
     * @param string     $field       Field to apply between condition on
     * @param int|string $value_start Minimum value for between condition
     * @param int|string $value_end   Maximum value for between condition
     * @param string     $operator    'AND'|'OR' - operator to combine with other conditions
     *
     * @return void
     */
     ...
     *
     * @param string $field Field to disable autosearch on
     *
     * @return void
     */
     ...
     *
     * @param string       $field        Field to add compare condition on
     * @param mixed[]      $search_value Value to compare with
     * @param string|false $partialMatch When true, partial match only required
     * @param string       $operator     'OR'|'AND' - Operator to compare with other conditions
     * @param bool         $escape       When true, escape the search value
     *
     * @return void
     */
    public function addRelationCompare($field, $search_value, $partialMatch=false, $operator='AND', $escape=true)
    {
        ...
        if (!is_object($search_value)) {
            ...
            foreach (array_reverse(preg_split('/\./', $relation)) as $r) {
                if ($r==='') {
                    continue; // Local table, no column.
                }
                if ($w!==null) {
                    $w=[$r=>['with'=>$w,'select'=>[]]];
                } else {
    ...
    {
        //Yii::trace(CVarDumper::dumpAsString([$name,$parameters],10,true),'relatedsearchbehavior.autoScope');
        /**
         * @var CActiveRecord $owner
         */
        $owner = $this->getOwner();
        ...
        if (!count($parameters)) {
            if (strpos($name, 'get') === 0) {
                ...
                $isAutoScope=($inOwner=$owner->hasAttribute($name)) ||
                         isset($this->relations[$name]);
                if ($isAutoScope) {
                    $message='Parameters required for autoscope "{class}.{property}';
                } else {
